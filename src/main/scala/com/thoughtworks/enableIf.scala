...
object enableIf {
  ...
  def classpathContains(classpathPart: String): Context => Boolean = { c =>
    c.classPath.exists(_.getPath.contains(classpathPart))
  }

  def classpathMatches(regex: Regex): Context => Boolean = { c =>
    c.classPath.exists { dep =>
      regex.pattern.matcher(dep.getPath).matches()
    }
  }

  def classpathMatchesArtifact(
      artifactId: String,
      version: String
  ): Context => Boolean = { c =>
    c.classPath.exists { dep =>
